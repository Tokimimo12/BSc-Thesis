...
def write_CSD(data,metadata,rootName,destination,compression,compression_opts,full_chunk_shape):
	...
	for metadataKey in metadata.keys():
		metadataHandle.create_dataset(metadataKey,(1,),dtype=h5py.special_dtype(vlen=unicode) if sys.version_info.major == 2 else h5py.special_dtype(vlen=str))
		cast_operator=unicode if sys.version_info.major == 2 else str
